...
async def api_register():
    try:
        ...
        image, name, username, password, mobile = data.get('image'), data.get(
            'name'), data.get('username'), data.get('password'), data.get('mobile')
...
async def api_login():
    try:
        ...
        if await srv.validate_user_async(username, password):
            payload = {'username': username, 'exp': datetime.datetime.now(
                datetime.timezone.utc) + config.JWT_EXPIRATION_DELTA}
            token = jwt.encode(
...
async def api_get_user_profile():
    ...
    if user:
        user_info = {"photo": user.get("Photo"), "name": user.get(
            "Name"), "username": user.get("Username"), "mobile": user.get("Contact")}
...
async def api_update_user_profile():
    data = await request.get_json()
    image, name, mobile = data.get(
        'image'), data.get('name'), data.get('mobile')
    if not name:
...
async def api_create_contact():
    ...
    success, message, new_contact = await srv.add_contact_async(
        ...
        data.get('labels', []), datetime.datetime.now(
...
async def api_edit_contact(contact_id):
    ...
    if request.method == 'PUT':
        ...
        success, message = await srv.update_contact_async(
            g.username, contact_id, new_name, data.get(
...
async def api_edit_label():
    data = await request.get_json()
    old_label, new_label = data.get(
        'old_label_name'), data.get('new_label_name')
